(function() {
	...
	*/
	...
	_.reduce = function(collection, iterator, accumulator) {
		var argsCorrect = (arguments.length === 2);
	...
	_.every = function(collection, iterator) {
		...
		return Boolean(_.reduce(collection, function(accumulator, value){
	...
	*/
	...
	_.extend = function(obj) {
		_.each(arguments, function(temp)	{
			for (var key in temp)	{
				obj[key] = temp[key];
			}
		});
	...
	*/
	...
	_.shuffle = function(array) {
		...
		var shuffle;
		while (current) {
			shuffle = Math.floor(Math.random() * current);
			current -= 1;
			temp = out[current];
			out[current] = out[shuffle];
			out[shuffle] = temp;
		}
		return out;
	};
